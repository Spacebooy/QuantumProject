...
function Mission({ missionIndex, mode, onExit, onComplete }) {
  ...
  function backtrack() {
    if (busy || stopped || !node.parent) return;
  ...
  return <main className="pathfinder">
    <header className="mission-hud">
      <div><button className="text-button" onClick={onExit} disabled={busy}>← All missions</button><h1>Quantum Path Finder</h1><p>{missions[missionIndex].name} <span> / {modes[mode][0]}</span></p></div>
    ...
    {stopped ? <section className="mission-end"><p className="eyebrow">{won ? 'ACCESS TO THE OUTSIDE' : 'SIGNAL LOST'}</p><h2>{won ? 'Escape successful.' : 'Out of lives.'}</h2>{won && <p className="rating" aria-label={`${rating} out of 3 stars`}>{'★'.repeat(rating)}{'☆'.repeat(3-rating)}</p>}<p>{stats.moves} moves · {stats.oracles} oracle calls · {stats.measurements} measurements · {stats.energy} energy</p><button className="btn btn-primary" onClick={onExit}>Choose another mission</button></section> : choices.length === 0 ? <section className="mission-end"><h2>A quiet dead end.</h2><p>No bomb here, but no way forward. Your explored route stays on the map.</p><button className="btn btn-primary" onClick={backtrack}>← Backtrack</button></section> : <section className="quantum-console" aria-label="Quantum scanner">
      <div className="console-top"><div><p className="eyebrow">QUANTUM SCANNER</p><h2>{choices.length} corridors ahead</h2><p>Choose how many qubits you need.</p></div><div className="allocate-controls"><label htmlFor="mission-qubits">Qubits</label><select id="mission-qubits" value={qubits} disabled={busy || allocated} onChange={e => { setQubits(+e.target.value); setTarget(0); setControl(+e.target.value > 1 ? 1 : 0); }}>{[1,2,3,4].map(n => <option key={n} value={n}>{n} → {2 ** n} states</option>)}</select>{!allocated ? <button className="btn btn-primary" onClick={allocate} disabled={busy}>Allocate scanner</button> : <button className="btn btn-secondary" onClick={clearScanner} disabled={busy}>Change allocation</button>}</div><div className="console-utilities"><button className="text-button" onClick={hint} disabled={busy}>Hint</button><button className="text-button" onClick={undo} disabled={!canOperate || !batches.length}>Undo</button><button className="text-button" onClick={reset} disabled={!canOperate}>Reset scanner</button></div></div>
      ...
      <div className="quantum-actions">
        {mode === 'beginner' && <button disabled={!canOperate} onClick={() => apply('Superposition', allGates(qubits,'H'))}><span className="action-glyph">H</span><span><b>Superposition</b><small>Apply H to every qubit</small></span></button>}
        <button disabled={!canOperate} onClick={() => apply('Safety oracle', oracle(qubits, safeChoices(maze,current)), 'oracle')}><span className="action-glyph">◎</span><span><b>Mark safe paths</b><small>Flip the marked phases</small></span></button>
        {mode !== 'advanced' && <button disabled={!canOperate} onClick={() => apply('Diffusion', diffusion(qubits), 'diffusion')}><span className="action-glyph">∿</span><span><b>Amplify</b><small>Interfere the amplitudes</small></span></button>}
        <button className="measure-action" disabled={!canOperate} onClick={measure}><span className="action-glyph">↗</span><span><b>Measure & move</b><small>Commit to one corridor</small></span></button>
      </div>
